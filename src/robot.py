...
class RobotPacket:
    ...
    task: str; current_goal: Point | None; current_path_digest: list[Point]
...
class RobotAgent:
    ...
    def step_predict_and_move(self, world: World, dt: float) -> None:
        ...
        self.true_path.append((float(self.true_pose[0]),float(self.true_pose[1]))); self.estimator.predict_from_command(executed_v,executed_omega,dt)
    ...
    def make_packet(self,time_s:float)->RobotPacket:
        ...
        return RobotPacket(self.id,float(time_s),self.map.make_digest(self.id,time_s),self.graph.make_digest(self.id,time_s),target_dict,self.current_task,self.current_goal,self._path_digest(),self.est_pose,self.cov_trace)
    ...
    def _expire_stale_teammate_intent(self,time_s:float)->None:
        ...
        for rid in stale:
            for d in (self.known_teammate_last_seen,self.known_teammate_goals,self.known_teammate_paths,self.known_teammate_tasks,self.known_teammate_pose,self.known_teammate_cov): d.pop(rid,None)
